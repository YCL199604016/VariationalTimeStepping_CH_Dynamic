...
+Fan.Trace()*theta3*dS+T_Delta*Diff_Gamma_CF*grad(Mu_Gamma).Trace()*grad(theta3).Trace()*dS-Init_Fan.Trace()*theta3.Trace()*dS\
...
for i in range(TLen-1):
    ...
    Mu_Gamma_int=Integrate(InnerProduct(grad(sol.components[2]).Trace(),grad(sol.components[2]).Trace()),mesh,definedon=mesh.Boundaries('outer_edges'),order=2)
